...
def generate():
    ...
    scenarios = [
        ("rede_logistica_30", 30, 0.10, 2026, False),
        ("rede_urbana_60", 60, 0.065, 2027, False),
        ("rede_regional_100", 100, 0.035, 2028, False),
        ("rede_desconexa_40", 40, 0.08, 2029, True),
    ]
    manifest = []
    for name, n, probability, seed, disconnected in scenarios:
        ...
        edges = {}
        groups = (
            [vertices[:25], vertices[25:38], [vertices[38]], [vertices[39]]]
            if disconnected
            else [vertices]
        )
        for group in groups:
        ...
        payload = {
            "vertices": vertices,
            "edges": [
                {"source": u, "target": v, "weight": w} for (u, v), w in sorted(edges.items())
            ],
        }
        ...
        manifest.append(
            {
                ...
                "strongly_connected": not disconnected,
            }
        )
    return manifest
