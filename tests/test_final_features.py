...
def test_random_graphs_against_independent_baseline(seed):
    ...
    graph = NetworkXGraph()
    for i in range(8): graph.add_vertex(str(i))
    # DAG permite pesos negativos sem criar ciclos negativos.
    for i in range(8):
        for j in range(i+1, 8):
            if rng.random() < .5: graph.add_edge(str(i), str(j), rng.randint(-5, 9))
    ...
    for u in a.vertices:
        for v in a.vertices:
            ...
            else: assert a.distance(u,v) == inf
...
def test_negative_self_loop_and_disconnected_cycle(solver):
    ...
    g.add_vertex('A')
    g.add_edge('Z', 'Z', -1)
    with pytest.raises(NegativeCycleError): solver.solve(g)
...
@pytest.mark.parametrize('name,n', [('rede_logistica_30.json',30),('rede_urbana_60.json',60),('rede_regional_100.json',100),('rede_desconexa_40.json',40)])
